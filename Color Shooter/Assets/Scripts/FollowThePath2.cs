...
public class FollowThePath2 : MonoBehaviour {
    ...
    public void SetPath()
    {
        ...
        //lastDirection = (path[path.Length - 1].position - path[path.Length - 2].position).normalized;
        lastDirection = (path[path.Length - 1].position - path[path.Length - 2].position);
    ...
    private void Update()
    {
        if (movingIsActive)
        {
            ...
            if (currentPathPercent > 1)                    //when the path is complete
            {
                ...
                {
                    GetComponent<Rigidbody2D>().velocity = lastDirection * speed * Time.deltaTime * 1.75f;
